...
Stage 00 now leans harder into the pixel-sim vibe while widening the map and adding denser, randomized obstacles. Use this checklist to exercise every system, confirm nothing regressed, and capture feedback before we lock the scaffold.
...
- **W / A / S / D** – Move the player around the map; collisions should stop you from walking through structures, rocks, or workers.
...
1. The grid is now 38×20 tiles. Each page load should shuffle:
   - A 5×5 MCS build (repositioned to a fresh valid spot every load).
   - 2×2 Starbucks, wood house, and dorm structures (no overlaps).
   - A small field of rectangular, square, or circular obstacles (ponds, rocks, fountains) that always leave at least one route between zones.
2. Refresh the page several times to confirm the shuffles respect spacing, feel noticeably denser than before, and still allow navigation between every landmark.
3. Verify the player cannot clip through any structure, rock, or worker regardless of the layout.
...
- Bumping into a worker should feel identical to colliding with a building—they now block the player while they are on the map.
...
- The grass, buildings, rocks, and characters should all present as crisp pixel art with no antialiasing (note the new tile patterns on grass, facades, and obstacles).
